# PHP Version Manager (PHPVM) - Complete Project Specification

## Project Overview

Build a shell-based PHP version manager for Linux systems (Ubuntu/Debian, RHEL/Fedora/CentOS, including WSL). The tool manages multiple PHP versions (5.6 to 8.5), extensions, PHP-FPM services, and per-version Composer installations with an interactive TUI using **gum** (https://github.com/charmbracelet/gum).

**Target Platforms:**
- Ubuntu/Debian (using Ondřej Surý's PPA)
- RHEL/Fedora/CentOS (using Remi repository)
- WSL (Windows Subsystem for Linux)

**Key Design Principles:**
- Single command entry point: `php` (replaces system php)
- Interactive menus using **gum** with beautiful TUI
- Works for both regular users and root user
- Supports system-wide and user-local installation modes
- Per-user configuration and composer packages
- No symlink manipulation - calls versioned binaries directly (e.g., `/usr/bin/php8.4`)
- Auto-switch PHP versions based on project `.phpversion` file
...
```
PHPVM_VERSION="1.0.0"
PHPVM_REPO="professor93/phpvm"  # GitHub repository for self-update
```
...
```
/usr/local/bin/
├── php                     # Main script (our wrapper)
└── composer                # Composer wrapper

/etc/
├── phpversion              # System-wide default version (optional)
└── profile.d/
    └── phpvm.sh            # Shell environment (sourced for all users)
```
...
~/.config/phpvm/
└── env.sh                  # Shell environment (sourced from shell rc)
```

### Per-User Data (Both Installation Modes)
```
~/.phpversion/              # Per-user directory
├── config                  # User's default PHP version (just "8.4")
└── {version}/              # Per-version data
    └── composer/
        ├── composer.phar   # Composer binary
        ├── composer.json   # Global packages config
        └── vendor/
            └── bin/        # Global package binaries (added to PATH)

/project/.phpversion        # Per-directory version file (just "8.4")
```
...
```
src/
├── php.sh                  # Main entry point (combines all modules)
├── modules/
│   ├── config.sh           # Configuration variables and constants
│   ├── colors.sh           # Color definitions and setup
│   ├── utils.sh            # Utility functions (msg, warn, error, success, timestamps)
│   ├── platform.sh         # Platform/distro detection and package manager abstraction
│   ├── ui.sh               # UI functions using gum (with fallbacks)
│   ├── version.sh          # Version detection and resolution
│   ├── install.sh          # Install/uninstall PHP versions and extensions
│   ├── use.sh              # Switch PHP versions (use command)
│   ├── list.sh             # List versions and extensions
│   ├── info.sh             # Show PHP information
│   ├── config_edit.sh      # Edit PHP configuration files
│   ├── fpm.sh              # FPM management
│   ├── which.sh            # Show active PHP and resolution source
│   ├── self_update.sh      # Self-update functionality
│   └── help.sh             # Help command
├── composer.sh             # Composer wrapper script
├── install.sh              # Installer script
└── env.sh                  # Shell environment template
```
...
### 1. config.sh - Configuration Constants
...
### 4. platform.sh - Platform Detection
...
setup_php_repository() {
    ...
    case "$pm" in
        apt)
            ...
            run_privileged apt-get update -qq
            success "Ondřej's PHP PPA configured"
            ;;
        dnf)
            # Fedora
...
### 5. ui.sh - User Interface with Gum
...
### 6. version.sh - Version Detection
...
### 7. install.sh - Installation Module
...
### 11. config_edit.sh - Configuration Editing
...
cmd_config() {
    ...
    local choice
    choice=$(gum_menu "Select configuration to edit:" \
        "CLI php.ini" \
        "FPM php.ini" \
        "FPM Pool (www.conf)")
...
cmd_which() {
    ...
    echo ""
    echo "${BOLD}Active PHP:${RESET} $version"
    echo "${BOLD}Binary:${RESET} $php_bin"
    echo "${BOLD}Source:${RESET} $source"
    echo ""
    
    echo "${BOLD}Resolution Order:${RESET}"
    echo ""
    ...
    # 1. Session
    if [[ -n "${PHPVERSION_USE:-}" ]]; then
        echo "  ${GREEN}✓${RESET} 1. Session (PHPVERSION_USE=${PHPVERSION_USE})"
        active_found=true
    else
    ...
    if [[ -n "$local_file" && "$active_found" == "false" ]]; then
        ...
        echo "  ${GREEN}✓${RESET} 2. Local (${local_file} = ${local_version})"
        active_found=true
    ...
    if [[ -f "$PHPVM_CONFIG" ]]; then
        ...
        if [[ "$active_found" == "false" ]]; then
            echo "  ${GREEN}✓${RESET} 3. User (~/.phpversion/config = ${user_version})"
            active_found=true
        else
    ...
    if [[ -f "$SYSTEM_CONFIG" ]]; then
        ...
        if [[ "$active_found" == "false" ]]; then
            echo "  ${GREEN}✓${RESET} 4. System (/etc/phpversion = ${system_version})"
            active_found=true
        else
    ...
    # 5. Fallback
    if [[ "$active_found" == "false" ]]; then
        echo "  ${GREEN}✓${RESET} 5. Fallback (first installed)"
    else
...
cmd_self_update() {
    ...
    local installer_url="${PHPVM_RAW_URL}/install.sh"
...
${BOLD}Information:${RESET}
  version             Show PHPVM and current PHP version info
  info                Show detailed PHP information
  which               Show active PHP and resolution source
...
### 16. version_cmd.sh - Version Command

```bash
cmd_version() {
    local php_version
    php_version=$(get_current_version)
    
    echo ""
    echo "${BOLD}PHP Version Manager (PHPVM)${RESET}"
    ...
    if [[ -n "$php_version" ]]; then
        ...
        echo "${BOLD}Current PHP:${RESET}"
        "$php_bin" -v 2>/dev/null | head -3 | sed 's/^/  /'
        echo ""
        echo "  Binary: $php_bin"
        echo "  Source: $source"
    else
        echo "${BOLD}Current PHP:${RESET}"
        echo "  ${YELLOW}No PHP version installed${RESET}"
...
# Source configuration (will be inlined in final build)
# source /path/to/modules/config.sh
# source /path/to/modules/colors.sh
# source /path/to/modules/utils.sh
# source /path/to/modules/platform.sh
# source /path/to/modules/version.sh
# source /path/to/modules/ui.sh
...
## Shell Environment (env.sh)

This file is installed to `/etc/profile.d/phpvm.sh` (system-wide) or `~/.config/phpvm/env.sh` (user-local):
...
php() {
    ...
    if [[ "$1" == "use" ]]; then
        ...
        if [[ -f "$PHPVM_SESSION_FILE" && -s "$PHPVM_SESSION_FILE" ]]; then
            ...
            __phpvm_update_path
            echo -e "\033[0;32m✓\033[0m PHP $PHPVERSION_USE set for this session"
        fi
...
select_install_mode() {
    ...
    if [[ $EUID -eq 0 ]]; then
        echo ""
        echo "${BOLD}Installation Mode${RESET}"
        echo ""
        echo "  1) System-wide (/usr/local/bin) - Available to all users"
        echo "  2) User-local (~/.local/bin) - Current user only"
        echo ""
...
do_install() {
    ...
    # Install gum
    install_gum || warn "gum installation failed, will use fallback menu"
...
## Main Script Entry Point (php.sh)
...
# [All modules would be sourced or inlined here]
...
|---------|-------------|
| `php` | Show help |
| `php version` | Show PHPVM version and current PHP version (detailed) |
| `php use` | Interactive version selection with scope |
| `php use X.Y` | Switch to specific version with scope selection |
| `php install` | Interactive install (PHP or extension) |
| `php install X.Y` | Install specific PHP version |
| `php list` | List installed PHP versions |
| `php list extensions` | List extensions for current PHP |
| `php info` | Show detailed PHP information |
| `php config` | Edit PHP configuration files |
| `php which` | Show active PHP and resolution source |
| `php fpm` | Manage PHP-FPM services and pools |
| `php self-update` | Update PHPVM to latest version |
| `php help` | Show help |
| `php -v` | Pass through to PHP binary |
| `php --version` | Pass through to PHP binary |
| `php [args]` | Pass through to current PHP binary |
| `composer [args]` | Run Composer with current PHP |
...
- **If root**: Ask "System-wide" or "User-local"
- **If not root**: User-local only (automatically)
- **System-wide**: `/usr/local/bin`, `/etc/profile.d/phpvm.sh`
- **User-local**: `~/.local/bin`, `~/.config/phpvm/env.sh`

### Gum Installation Priority
1. Try GitHub releases (download binary)
2. If fails, try package manager (apt/dnf charm repo)
3. If fails, use fallback numbered menu

### Auto-Switch Notification
When `cd` into a directory with `.phpversion`:
```
→ Switched to PHP 8.3 (.phpversion)
```

### Version File Format
- `.phpversion` contains only version number: `X.Y` (e.g., `8.4`)
- No comments, no ranges, no partial versions
- Trimmed whitespace

### Composer Installation
- Per-PHP-version in `~/.phpversion/{version}/composer/`
- **Not** installed automatically
- Prompt with yes/no on first `composer` command

### Error Handling
- Never use `set -e`
- Show clear error messages with red ✗
- Provide actionable solutions
- Check package availability before attempting install
- Validate binary exists after installation

### Logging
- Timestamps on important operations (INFO level)
- Log file: `~/.phpversion/phpvm.log`
- Color-coded output

### Shell Support
- bash 4.0+
- zsh 5.0+
- Auto-detect and configure appropriate rc file
...
### Installation
- [ ] Fresh install on Ubuntu (user-local)
- [ ] Fresh install on Ubuntu (system-wide as root)
- [ ] Fresh install on Fedora/RHEL
- [ ] Fresh install in WSL
- [ ] Gum installs from GitHub
- [ ] Fallback menu works when gum fails
- [ ] Uninstall removes all files

### PHP Management
- [ ] Install PHP 8.4 (first install, sets default)
- [ ] Install PHP 8.3 (second install)
- [ ] `php use 8.3` → Session
- [ ] Verify `php -v` shows 8.3
- [ ] New terminal defaults back
- [ ] `php use` → Local directory
- [ ] Auto-switch notification on `cd`
- [ ] `php install` → Extension
- [ ] `php version` shows tool + PHP version
- [ ] `php self-update` checks GitHub

### Composer
- [ ] First `composer` prompts to install
- [ ] Composer per-version isolation works
- [ ] Global packages in PATH
...
### Cross-Platform
- [ ] Works on Debian/Ubuntu (apt)
- [ ] Works on RHEL/Fedora (dnf)
- [ ] Repository auto-setup works
- [ ] Package naming differences handled
...
# Combine all modules into php
{
    echo '#!/bin/bash'
    echo '# PHP Version Manager (PHPVM) v1.0.0'
    echo '# https://github.com/professor93/phpvm'
    echo ''
    cat src/modules/config.sh
    cat src/modules/colors.sh
    cat src/modules/utils.sh
    cat src/modules/platform.sh
    cat src/modules/ui.sh
    cat src/modules/version.sh
    cat src/modules/install.sh
    cat src/modules/use.sh
    cat src/modules/list.sh
    cat src/modules/info.sh
    cat src/modules/config_edit.sh
    cat src/modules/fpm.sh
    cat src/modules/which.sh
    cat src/modules/self_update.sh
    cat src/modules/version_cmd.sh
    cat src/modules/help.sh
    cat src/php.sh | tail -n +3  # Skip shebang
} > dist/php
...
# Composer wrapper
{
    ...
    cat src/modules/ui.sh
    cat src/modules/version.sh
    cat src/composer.sh | tail -n +3
} > dist/composer

# Environment
cp src/env.sh dist/env.sh

# Make executable
chmod +x dist/php dist/composer
...
```
✗ PHP repository not configured

Would you like to set up the PHP repository? (Y/n)

Manual setup instructions:
  Ubuntu/Debian: sudo add-apt-repository ppa:ondrej/php
  RHEL/Fedora:   sudo dnf install https://rpms.remirepo.net/...
```

### Package Not Available
```
✗ PHP 8.5 is not available in the repository
...
Try running: sudo apt-get update (or sudo dnf check-update)
```

### Binary Not Found After Install
```
⚠ PHP 8.4 was installed but binary not found at expected location
⚠ You may need to configure your PATH or check the installation
```
...
Would you like to install PHP now? (Y/n)
```